...
function List({ users, handleDelete, role, permission }) {
    ...
    return (
        <div className='contain-table'>
            <div style={{ marginTop: '30px', fontWeight: '200', fontSize: '18px', color: 'white', backgroundColor: '#4b8df8', width: '100%', padding: '7px 15px' }}>
                <BusinessCenterIcon style={{ fontSize: '20px', }} /> Users List
            </div>

            <div style={{ marginTop: '', border: '1px solid #4b8df8', padding: '10px' }}>

                <TableContainer>
                    <Table sx={{ minWidth: '100%' }} size="small" aria-label="Users List">
                        <TableHead>
                            <TableRow>
                                ...
                                users.map((user, i) => (
                                    <TableRow key={user.id}>
                                        ...
                                        <TableCell className="text-left">
                                            ...
                                            <button
                                                onClick={() => openDeleteDialog({ id: user.id, username: user.username })}
            ...
            <DeleteDialog
                title='Delete?'
                content={`Do you wish to delete user ${deleteId ? deleteId.username : null}`}
                titleYes='Delete'
                open={openDelete}
                handleAction={() => {
