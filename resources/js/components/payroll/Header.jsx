...
function Header({ setPayHeads, role, permission }) {

    const Temp = async (e) => {
        let key = e.target.value;
        let result = await fetch(`/api/payheads/${key}`);
    ...
    return (
        <header >
            <p style={{ fontWeight: '300', fontSize: '40px' }}>Payrolls</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div >
                    {checkPermission("add payroll") ? (

                    <Link className='' style={{ display: 'flex', alignItems: 'center', fontSize: '14px', borderRadius: 'none', 'background': '#35aa47', 'color': "white", "padding": "7px 20px", 'textDecoration': 'none' }} to={'/payrolls/add'}>Add New Payroll <ControlPointIcon style={{ padding: '3px' }} /></Link>
                    ) : null}
