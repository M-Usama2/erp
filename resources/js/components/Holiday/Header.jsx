...
function Header({ setHolidays, role, permission }) {

    const Temp = async (e) => {
        let key = e.target.value;
        let result = await fetch(`/api/holidays/${key}`);
    ...
    return (
        <header >
            <p style={{ fontWeight: '300', fontSize: '40px' }}>Holiday</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div >
                    {checkPermission("add holiday") ? (

                        <Link className='' style={{ display: 'flex', alignItems: 'center', fontSize: '14px', borderRadius: 'none', 'background': '#35aa47', 'color': "white", "padding": "7px 20px", 'textDecoration': 'none' }} to={'/holidays/add'}>Add New Holiday <ControlPointIcon style={{ padding: '3px' }} /></Link>
                    ) : null}
