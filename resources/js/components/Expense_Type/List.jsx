...
function List({ expenseType, token, fecthData, role , permission }) {
    ...
    return (
        <div className='contain-table'>
            <div style={{ marginTop: '30px', fontWeight: '200', fontSize: '18px', color: 'white', backgroundColor: '#4b8df8', width: '100%', padding: '7px 15px' }}>
                <BusinessCenterIcon style={{ fontSize: '20px', }} /> Expense Type List
            </div>
                                ...
                                expenseType.map((expense, i) => (
                                    <TableRow key={expense.id}>
                                        ...
                                        <TableCell className="text-left">
                                                ...
                                                <Link
                                                to={`/expense_type/update/${expense.id}`}
                                                ...
                                                <button
                                                onClick={() => openDeleteDialog({ id: expense.id })}
            ...
            <DeleteDialog
                title='Delete?'
                content={`Do you wish to delete Expense_Type ${deleteId ? deleteId.id : null}`}
                titleYes='Delete'
                open={openDelete}
                handleAction={() => {
