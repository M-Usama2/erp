    ...
    async function Submit(e) {
        ...
        try {
            const response = await axios({
                ...
                if (e.data.data.id) {
                    navigate(`/employees/update/${e.data.data.id}`);
                }
            })
        } catch (err) {
            toast.error(err)
    ...
    return (
        <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', padding: '30px' }}>
            <div style={{ width: '90%', border: '1px solid #4b8df8', padding: '0px', }}>
                <div style={{ background: '#1CAF9A', height: '60px', color: 'white', display: 'flex', alignItems: 'center', padding: '30px' }}>
                    <h6 className='text-center'>CREATE EMPLOYEE</h6>
                </div>
