...
function Header({ setAttendanceSettings }) {

    const Temp = async (e) => {
        let key = e.target.value;
        let result = await fetch(`/api/attendance_settings/${key}`);
    ...
    return (
        <header >
            <p style={{ fontWeight: '300', fontSize: '40px' }}>Attendance Setting</p>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div >
                    <Link className='' style={{ display: 'flex', alignItems: 'center', fontSize: '14px', borderRadius: 'none', 'background': '#35aa47', 'color': "white", "padding": "7px 20px", 'textDecoration': 'none' }} to={'/attendance/settings/add'}>Add New Job Time <ControlPointIcon style={{ padding: '3px' }} /></Link>
